...
def load_raw():
    ...
    # Take the number of rows for the pilot participant
    rows_per_participant = df.loc[df['participant'] == 998].shape[0]

    numof_participants_with_missing_participant_number = df.loc[
        df['participant'].isna(), 'date'].unique().shape[0]

    # Assert that no two participants with missing participant number started
    # the experiment at the same time
    assert df.loc[df['participant'].isna()].shape[0] == (rows_per_participant
                                                         * numof_participants_with_missing_participant_number)

    # remove pilot runs or participants which did not enter the participant number
    df = df.loc[
        (~df['participant'].isin([998, 999])) & (df['participant'].notna())]
    ...
    # remove one participant who did not complete the experiment
    df = df.loc[~df[c.COLUMN_NAME_UID].isin(['240:2022_Dec_14_0915'])]

    return df


def handle_task(df):
    task_df = df.copy().rename(
        columns={
            'task_key_resp.rt': c.COLUMN_NAME_RAW_RESP_TIME,
            'response_accuracy': c.COLUMN_NAME_RESP_ACCURACY,
            'effect_patch': c.COLUMN_NAME_FEEDBACK_CYCLE
        }
    )
    ...
    task_df.loc[~task_df['demo_trials_loop.thisRepN'].isna()].to_csv(
